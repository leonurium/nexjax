...
function readConfig(projectRoot) {
  ...
  // Read permission strings from app.json (Expo) or existing config
  let permissions = {};
  if (appJson?.expo?.ios?.infoPlist) {
    // Extract all permission strings from app.json infoPlist
    const infoPlist = appJson.expo.ios.infoPlist;

    // Extract all permission strings from infoPlist
    // iOS permission keys typically end with "UsageDescription"
    Object.keys(infoPlist).forEach((key) => {
      // Match permission keys (end with UsageDescription - standard iOS pattern)
  ...
  // Remove undefined permissions to keep config clean
