...
/**
 * Find Info.plist file (prioritizes app's Info.plist, not Pods/frameworks)
 */
function findInfoPlist(iosPath) {
  // 1. App Info.plist: ios/<AppName>/Info.plist (AppName = xcodeproj name)
  const files = fs.readdirSync(iosPath);
  const xcodeproj = files.find((f) => f.endsWith(".xcodeproj"));
  if (xcodeproj) {
    const appName = xcodeproj.replace(/\.xcodeproj$/, "");
    const appPlist = path.join(iosPath, appName, "Info.plist");
  ...
  // 3. Search recursively, but exclude Pods (framework plists are not the app's)
  const findInDir = (dir) => {
    try {
      const entries = fs.readdirSync(dir, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.name === "Pods") continue; // Skip Pods - contains framework plists
